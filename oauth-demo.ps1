...
Write-Host "`n🧪 Testing server..." -ForegroundColor Blue
try {
...
Write-Host "`n🔗 OAuth Endpoints Available:" -ForegroundColor Blue
...
# Test OAuth redirect
Write-Host "`n🧪 Testing Google OAuth redirect..." -ForegroundColor Blue
try {
...
Write-Host "`n🌐 Opening Google OAuth flow in browser..." -ForegroundColor Yellow
Start-Process "http://localhost:8080/oauth2/authorization/google"

Write-Host "`n📋 OAuth Flow Steps:" -ForegroundColor Blue
...
Write-Host "`n✅ OAuth demo is running!" -ForegroundColor Green
